...
rematch <- read.csv("~/shared_drive/Personal/Deliang Yang/IBD_CD/cost_op_lib_rematch_ibd_cd.csv") %>% filter(fl == 1)
...
# 7.1 Save as CSV ----
write.csv(results_all_years, "./results/IBD_CD_Cost_Analysis_2003_2022_v2.csv", row.names = FALSE)
...
write.csv(wide_format_results, "./results/IBD_CD_Cost_Summary_Wide_Format_v2.csv", row.names = FALSE)
...
compare_arima_models <- function(data, training_years, forecast_years) {
  ...
  get_top_models <- function(data_series, ic = "aic", xreg = NULL) {
    ...
    if(selected_is_second_best) {
      # Try to refit with the selected p and original d,q
      current_order <- arimaorder(model)
      new_order <- c(selected_p, current_order[2:3])
      
      # Try to fit the new model, but catch errors
      tryCatch({
        # Extract full ARIMA order from second best model name
        if(grepl("ARIMA\\(([0-9]+),([0-9]+),([0-9]+)\\)", second_best_name)) {
          order_match <- regmatches(second_best_name, regexec("ARIMA\\(([0-9]+),([0-9]+),([0-9]+)\\)", second_best_name))[[1]]
          if(length(order_match) >= 4) {
            new_order <- c(as.numeric(order_match[2]), as.numeric(order_match[3]), as.numeric(order_match[4]))
          }
        }
        ...
        cat(sprintf("\nError fitting second best model: %s", e$message))
        cat("\nReverting to auto.arima's best model.\n")
        selected_model <<- model
        model_info_text <<- sprintf("Best model kept due to refit error (gap=%.4f, p=%d)", gap, best_p)
        selected_is_second_best <<- FALSE
      })
    } else {
...
write.csv(combined_forecasts, "./results/IBD_CD_Cost_Forecasts_2023_2032_v2.csv", row.names = FALSE)
...
# Save combined data
write.csv(combined_data, "./results/IBD_CD_Costs_Historical_and_Forecast_2003_2032_v2.csv", row.names = FALSE)
...
write.csv(combined_wide, "./results/IBD_CD_Costs_Wide_Format_2003_2032_v2.csv", row.names = FALSE)
...
write.xlsx(
  excel_output,
  file = "./results/IBD_CD_Cost_Analysis_and_Forecast_2003_2032_v3.xlsx",
  colNames = TRUE,
  borders = "columns",
  firstRow = TRUE
)
...
message("2. Historical data (2003-2022): ./results/IBD_CD_Cost_Analysis_2003_2022.csv")
message("3. Forecast data (2023-2032): ./results/IBD_CD_Cost_Forecasts_2023_2032.csv")
message("4. Combined historical and forecast: ./results/IBD_CD_Costs_Historical_and_Forecast_2003_2032.csv")
message("5. Complete Excel report: ./results/IBD_CD_Cost_Analysis_and_Forecast_2003_2032.xlsx")