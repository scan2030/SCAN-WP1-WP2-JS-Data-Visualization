# SCAN2030: CD Unmet Need Analysis ----------------------------------------
# Author: Yin Zhang, Deliang Yang
# Purpose: Calculate Unmet Need for CD
# Input: 
...
# Extract CD incident cases for analysis
incident <- ibd_dx %>% 
  filter(type == "CD") %>% 
  mutate(
...
# 6. MEDICATION ANALYSIS (CD PATIENTS) --------------------------------
# Extract CD patients for medication analysis
cd_dx <- ibd_dx %>% filter(type == "CD")

# Load prescription data
Rx <- readRDS("./IBD_2000-2024cohort_2000-2024Rx.RDS")%>%distinct()

# Select key fields from CD patients
cd_dx_selected <- cd_dx %>% dplyr::select(reference.key., firstDiag, date.of.registered.death.)

# Get prescriptions after diagnosis for CD patients
Rx_afterDx_cd <- Rx %>%
  filter(reference.key. %in% cd_dx_selected$reference.key.) %>% 
  left_join(cd_dx_selected, by = "reference.key.") %>%
...
steroidRx_cd_merged <- steroidRx_cd_sorted %>%
...
steroidRx_cd_merged <- steroidRx_cd_merged %>%
  mutate(
...
merged_data <- steroidRx_cd_merged %>%
...
steroidRx_cd_earliest <- steroidRx_cd_merged %>%
  group_by(reference.key.) %>%
  summarise(
...
merged_earliest <- full_join(steroidRx_cd_earliest, eligible3_earliest, by = "reference.key.") %>%
  mutate(
...
# Add eligibility information to CD patient data
cd_dx <- full_join(cd_dx, merged_earliest, by = "reference.key.")
...
IBDPx <- Px %>% 
  ...
  left_join(cd_dx, by = "reference.key.") %>% 
...
# Extract biologic medication data for CD patients
bioRx_cd <- Rx_afterDx_cd %>%
  ...
  filter(reference.key. %in% cd_dx$reference.key.)

# Combine procedure and biologic data
IBDPx_bioRx_cd <- IBDPx %>% 
  filter(reference.key. %in% cd_dx$reference.key.) %>% 
  left_join(bioRx_cd, by = "reference.key.")

# Identify procedures occurring before biologic therapy (surgical unmet need)
IBDPx_bioRx_cd <- IBDPx_bioRx_cd %>%
...
merged_earliest_Rx <- merged_earliest %>% full_join(bioRx_cd, by = "reference.key.")
...
IBDPx_bioRx_cd_filtered <- IBDPx_bioRx_cd %>%
  select(reference.key., firstDiag, procedure, Px_unmet)

# Merge all data sources
merged_data <- full_join(merged_earliest_Rx, IBDPx_bioRx_cd_filtered, by = "reference.key.") %>% 
  left_join(firstDx_death, by = "reference.key.")
...
firstdx <- cd_dx %>% select(reference.key., firstDiag)
merged_data <- merged_data %>% 
  select(-firstDiag) %>% 
...
write.xlsx(incidence_unmet_df, "/Users/zhangyin/Library/CloudStorage/OneDrive-connect.hku.hk/IBD/WP1/unmet needs/incidence_unmet_table_cd.xlsx", rowNames = FALSE)
write.xlsx(merged_data, "/Users/zhangyin/Library/CloudStorage/OneDrive-connect.hku.hk/IBD/WP1/unmet needs/incidence_unmet_cd.xlsx", rowNames = FALSE)
...
write.csv(incidence_summary, "IBD_CD Unmet Need Plot Data.csv")